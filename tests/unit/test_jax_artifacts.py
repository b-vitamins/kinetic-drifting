...
def test_load_mae_model_restores_jax_checkpoint_run_root_with_numerical_parity(
    tmp_path: Path,
) -> None:
    upstream_mae = _import_upstream("models.mae_model")
    metadata = {
    ...
    loaded_model, loaded_metadata = load_mae_model(str(run_root))
    loaded_model = cast(MAEResNet, loaded_model)
...
def test_load_generator_model_restores_jax_checkpoints_dir_with_numerical_parity(
    tmp_path: Path,
) -> None:
    upstream_utils = _import_upstream("utils.hsdp_util")
    upstream_utils.set_global_mesh(1)
    ...
    loaded_model, loaded_metadata = load_generator_model(str(run_root / "checkpoints"))
    loaded_model = cast(DitGen, loaded_model)
